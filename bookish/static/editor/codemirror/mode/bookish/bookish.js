(function (mod) {
    ...
    // Initialize typo with dictionaries
    var dictionary = new Typo("en_US");
    var customize_dictionary = new Typo("customize");

    // all the possible characters that can separate words
    var rx_word = "!\"#$%&()*+,-./:;<=>?@[\\]^_`{|}~ ";
    ...
    CodeMirror.defineMode("bookish", function (cmCfg, modeCfg) {
        ...
        function inText(stream, state) {
            ...
            if (state.startOfLine && stream.match("//")) {
                ...
                style = null;
                var ch = stream.peek();
                if (ch && !rx_word.includes(ch)) {
                    var word = "";
                    while (ch && !rx_word.includes(ch)) {
                        stream.next();
                        word += ch;
                        ch = stream.peek();
                    }
                    if (word !== "") {
                        // spell check
                        if (!dictionary.check(word) && !customize_dictionary.check(word)) {
                            style = "spell-error";
                        }
                    }
                } else {
